...
static u_char *
ngx_rtmp_log_var_sample_date_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    return ngx_slprintf(buf, buf + NGX_INT64_LEN,
            "%ui", s->delay->start_time + s->delay->time_cost);
}
...
static size_t
ngx_rtmp_log_var_sample_pts_getlen(ngx_rtmp_session_t *s,
    ngx_rtmp_log_op_t *op)
{
    return NGX_INT32_LEN;
}
...
static u_char *
ngx_rtmp_log_var_sample_pts_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    return ngx_slprintf(buf, buf+NGX_INT32_LEN, "%uD", (uint32_t)s->delay->cur_audio_pts);
}

static size_t
ngx_rtmp_log_var_sample_pts_diff_getlen(ngx_rtmp_session_t *s,
    ngx_rtmp_log_op_t *op)
{
    return NGX_INT32_LEN;
}
...
static u_char *
ngx_rtmp_log_var_sample_pts_diff_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    return ngx_slprintf(buf, buf + NGX_INT32_LEN, 
            "%uD",
            (uint32_t)(s->delay->cur_audio_pts - s->delay->last_audio_pts));
}

static size_t
ngx_rtmp_log_var_audio_duration_getlen(ngx_rtmp_session_t *s,
    ngx_rtmp_log_op_t *op)
{
    return NGX_INT32_LEN;
}
...
static u_char *
ngx_rtmp_log_var_audio_duration_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    return ngx_slprintf(buf, buf + NGX_INT32_LEN,
            "%uD", (uint32_t)s->delay->audio_duration);
}

static size_t
ngx_rtmp_log_var_time_cost_getlen(ngx_rtmp_session_t *s,
    ngx_rtmp_log_op_t *op)
{
    return NGX_INT32_LEN;
}
...
static u_char *
ngx_rtmp_log_var_time_cost_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    return ngx_slprintf(buf, buf + NGX_INT32_LEN,
            "%uD", (uint32_t)s->delay->time_cost);
}

static size_t
ngx_rtmp_log_var_video_frame_num_getlen(ngx_rtmp_session_t *s,
    ngx_rtmp_log_op_t *op)
{
    return NGX_INT32_LEN;
}
...
static u_char *
ngx_rtmp_log_var_video_frame_num_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    ngx_rtmp_live_ctx_t* ctx;
    ctx = ngx_rtmp_get_module_ctx(s, ngx_rtmp_live_module);
    ...
    return ngx_slprintf(buf, buf+NGX_INT32_LEN,
            "%uL", (uint64_t)s->delay->video_frame_num);
}

static size_t
ngx_rtmp_log_var_audio_delay_max_getlen(ngx_rtmp_session_t *s,
    ngx_rtmp_log_op_t *op)
{
    return NGX_INT64_LEN;
}
...
static u_char *
ngx_rtmp_log_var_audio_delay_max_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    return ngx_slprintf(buf, buf + NGX_INT64_LEN,
            "%i", s->delay->audio_delay_max);
}

static size_t
ngx_rtmp_log_var_audio_delay_min_getlen(ngx_rtmp_session_t *s,
    ngx_rtmp_log_op_t *op)
{
    return NGX_INT64_LEN;
}
...
static u_char *
ngx_rtmp_log_var_audio_delay_min_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    return ngx_slprintf(buf, buf + NGX_INT64_LEN,
            "%i", s->delay->audio_delay_min);
}

static size_t
ngx_rtmp_log_var_recv_delay_getlen(ngx_rtmp_session_t *s,
    ngx_rtmp_log_op_t *op)
{
    return NGX_INT64_LEN;
}
...
static u_char *
ngx_rtmp_log_var_recv_delay_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    return ngx_slprintf(buf, buf + NGX_INT64_LEN,
            "%i", s->delay->recv_delay);
}

static size_t
ngx_rtmp_log_var_send_delay_getlen(ngx_rtmp_session_t *s,
    ngx_rtmp_log_op_t *op)
{
    return NGX_INT64_LEN;
}
...
static u_char *
ngx_rtmp_log_var_send_delay_getdata(ngx_rtmp_session_t *s, u_char *buf,
    ngx_rtmp_log_op_t *op)
{
    return ngx_slprintf(buf, buf + NGX_INT64_LEN,
            "%i", s->delay->send_delay);
}
...
static ngx_rtmp_log_var_t ngx_rtmp_log_vars[] = {
    ...
    { ngx_string("end_role"),
      ngx_rtmp_log_var_end_role_getlen,
      ngx_rtmp_log_var_end_role_getdata,
      0 },

    { ngx_string("stream_id"),
      ngx_rtmp_log_var_stream_id_getlen,
      ngx_rtmp_log_var_stream_id_getdata,
      0 },

    { ngx_string("sample_date"),
        ngx_rtmp_log_var_sample_date_getlen,
        ngx_rtmp_log_var_sample_date_getdata,
        0 },

    { ngx_string("meta_x"),
        ngx_rtmp_log_var_meta_x_getlen,
        ngx_rtmp_log_var_meta_x_getdata,
        0 },

    { ngx_string("sample_pts"),
        ngx_rtmp_log_var_sample_pts_getlen,
        ngx_rtmp_log_var_sample_pts_getdata,
        0 },

    { ngx_string("sample_pts_diff"),
        ngx_rtmp_log_var_sample_pts_diff_getlen,
        ngx_rtmp_log_var_sample_pts_diff_getdata,
        0 },

    { ngx_string("audio_duration"),
        ngx_rtmp_log_var_audio_duration_getlen,
        ngx_rtmp_log_var_audio_duration_getdata,
        0 },

    { ngx_string("time_cost"),
        ngx_rtmp_log_var_time_cost_getlen,
        ngx_rtmp_log_var_time_cost_getdata,
        0 },

    { ngx_string("video_frame_num"),
        ngx_rtmp_log_var_video_frame_num_getlen,
        ngx_rtmp_log_var_video_frame_num_getdata,
        0 },

    { ngx_string("audio_delay_max"),
        ngx_rtmp_log_var_audio_delay_max_getlen,
        ngx_rtmp_log_var_audio_delay_max_getdata,
        0 },

    { ngx_string("audio_delay_min"),
        ngx_rtmp_log_var_audio_delay_min_getlen,
        ngx_rtmp_log_var_audio_delay_min_getdata,
        0 },

    { ngx_string("recv_delay"),
        ngx_rtmp_log_var_recv_delay_getlen,
        ngx_rtmp_log_var_recv_delay_getdata,
        0 },

    { ngx_string("send_delay"),
        ngx_rtmp_log_var_send_delay_getlen,
        ngx_rtmp_log_var_send_delay_getdata,
        0 },
...
static void
ngx_rtmp_log_delay(ngx_rtmp_session_t *s)
{
    ngx_rtmp_log_app_conf_t    *lacf;
    ...
    ctx->event = NGX_RTMP_LOG_EVENT_DELAY;
    while (s->delay) {

        log = lacf->logs->elts;
        for (n = 0; n < lacf->logs->nelts; ++n, ++log) {

            fmt = log->standards[NGX_RTMP_LOG_EVENT_DELAY];
            if (fmt) {
                ngx_rtmp_log_write(s, log, fmt);
            }
        }
        ...
        delay->next = s->delay_free;
        s->delay_free = delay;
    }
}

static void
ngx_rtmp_log_delay_timer(ngx_event_t *e)
{
    ngx_rtmp_log_ctx_t         *ctx;
    ngx_rtmp_session_t         *s;
