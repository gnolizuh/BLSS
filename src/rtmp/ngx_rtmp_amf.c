...
static ngx_inline ngx_rtmp_amf_elt_ex_t*
ngx_rtmp_amf_alloc(ngx_pool_t* pool, ngx_int_t type, size_t len) {
    ngx_rtmp_amf_elt_ex_t  *elt;

    elt = ngx_palloc(pool, sizeof(ngx_rtmp_amf_elt_ex_t));

    if (elt == NULL) {
        return NULL;
    }

    ngx_memzero(elt, sizeof(ngx_rtmp_amf_elt_ex_t));
    elt->next = NULL;
    elt->len = len;
    elt->data = ngx_palloc(pool, elt->len);
    ...
    elt->type = type;
    ngx_memzero(elt->data, elt->len);

    return elt;
}

static ngx_inline void
ngx_rtmp_amf_insert(ngx_rtmp_amf_ctx_t *ctx,
        ngx_rtmp_amf_elt_ex_t  *elt)
{
    elt->next = ctx->elts;
    ctx->elts = elt;
...
static ngx_int_t
ngx_rtmp_amf_read_object_ex(ngx_rtmp_amf_ctx_t *ctx,
        ngx_rtmp_amf_elt_t *elts, ngx_int_t nelts)
{
    u_char                 *name, buf[2];
    uint16_t                len;
    ngx_int_t               rc;
    ...
    while(ctx->elts) {
        elt_ex = ctx->elts->next;
        ngx_rtmp_amf_free(ctx->pool, ctx->elts);
        ctx->elts = elt_ex;
    }
    ...
    for ( ;; ) {
        ...
        name = ngx_palloc(ctx->pool, len);
        if (!name) {
            return NGX_ERROR;
        }
        ngx_memzero(name, len);
        rc = ngx_rtmp_amf_get(ctx, name, len);

        if (rc != NGX_OK) {
            ngx_pfree(ctx->pool, name);
            return NGX_ERROR;
        }

        for (n = 0; n < nelts && (len != elts[n].name.len
                || ngx_strncasecmp(name, elts[n].name.data, len));
                n++)
        {
        }

        if (ngx_rtmp_amf_read_ex(ctx, 
                    n < nelts
                    ? &elts[n]
                    : NULL, 1) != NGX_OK) {
            ngx_pfree(ctx->pool, name);
        ...
        if (n >= nelts && ctx->elts) {
            ctx->elts->name.len = len;
            ctx->elts->name.data = name;
        } else {
            ngx_pfree(ctx->pool, name);
        }
    }
...
static ngx_int_t
ngx_rtmp_amf_read_array_ex(ngx_rtmp_amf_ctx_t* ctx,
        ngx_rtmp_amf_elt_t *elts, ngx_int_t nelts)
{
    uint32_t    len, n;
    u_char      buf[4];
    ...
    for (n = 0; n < len; ++n) {
        if (ngx_rtmp_amf_read_ex(ctx, n < nelts
                    ? &elts[n]
                    : NULL, 1) != NGX_OK) {
            return NGX_ERROR;
        }
...
static ngx_int_t
ngx_rtmp_amf_read_variant_ex(ngx_rtmp_amf_ctx_t* ctx,
        ngx_rtmp_amf_elt_t *elts, ngx_int_t nelts)
{
    uint8_t     type;
    ngx_int_t   n;
    ...
    return ngx_rtmp_amf_read_ex(ctx, &elt, 1);
}

ngx_int_t
ngx_rtmp_amf_read_ex(ngx_rtmp_amf_ctx_t *ctx,
        ngx_rtmp_amf_elt_t *elts, size_t nelts)
{
    ngx_int_t               rc;
    void                   *data;
    ...
    u_char                  buf[8];
    uint32_t                n;
    int8_t                  conflict;/*type conflict with elts*/
    ngx_rtmp_amf_elt_ex_t  *elt_ex;
    ...
    for( n = 0 ; n < nelts ; n++) {
        ...
        if (elts && elts->type & NGX_RTMP_AMF_TYPELESS) {
            ...
            rc = ngx_rtmp_amf_get(ctx, &type8, 1);
            switch (rc) {
                case NGX_DONE:
                    return NGX_OK;
                case NGX_ERROR:
        ...
        switch (type) {
            case NGX_RTMP_AMF_NUMBER:
                ...
                if (data == NULL) {
                    elt_ex = ngx_rtmp_amf_alloc(ctx->pool, type, 8);
                    if (elt_ex == NULL) {
                        return NGX_ERROR;
                    }
            ...
            case NGX_RTMP_AMF_BOOLEAN:
                if (data == NULL) {
                    elt_ex = ngx_rtmp_amf_alloc(ctx->pool, type, 1);
                    if (elt_ex == NULL) {
                        return NGX_ERROR;
                    }
                    data = elt_ex->data;
            ...
            case NGX_RTMP_AMF_OBJECT:
                if (ngx_rtmp_amf_read_object_ex(ctx, data,
                    data && elts ? elts->len / sizeof(ngx_rtmp_amf_elt_t) : 0
                    ) != NGX_OK)
                {
                    return NGX_ERROR;
            ...
            case NGX_RTMP_AMF_ARRAY:
                if (ngx_rtmp_amf_read_array_ex(ctx, data,
                    data && elts ? elts->len / sizeof(ngx_rtmp_amf_elt_t) : 0
                    ) != NGX_OK)
                {
                    return NGX_ERROR;
            ...
            case NGX_RTMP_AMF_VARIANT_:
                if (ngx_rtmp_amf_read_variant_ex(ctx, data,
                    data && elts ? elts->len / sizeof(ngx_rtmp_amf_elt_t) : 0
                    ) != NGX_OK)
                {
                    return NGX_ERROR;
            ...
            case NGX_RTMP_AMF_INT16:
                ...
                if (!data) {
                    elt_ex = ngx_rtmp_amf_alloc(ctx->pool, type, 2);
                    if (elt_ex == NULL) {
                        return NGX_ERROR;
                    }
                }
            ...
            case NGX_RTMP_AMF_INT32:
                ...
                if (!data) {
                    elt_ex = ngx_rtmp_amf_alloc(ctx->pool, type, 4);
                    if (elt_ex == NULL) {
                        return NGX_ERROR;
                    }
                    data = elt_ex->data;
        ...
        if (elt_ex && !conflict) {
            ngx_rtmp_amf_insert(ctx, elt_ex);
        }
...
fail:
    if(elt_ex) {
        ngx_rtmp_amf_free(ctx->pool, elt_ex);
    }
    return NGX_ERROR;
}
