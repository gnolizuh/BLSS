...
static ngx_conf_enum_t                  ngx_rtmp_core_load_conf_slots[] = {
    { ngx_string("local"),              NGX_RTMP_CORE_LOAD_CONF_LOCAL  },
    { ngx_string("remote"),             NGX_RTMP_CORE_LOAD_CONF_REMOTE },
    { ngx_null_string,                  0 }
};
...
static ngx_command_t  ngx_rtmp_core_commands[] = {
    ...
    { ngx_string("rtmp_publish_domains"),
        NGX_RTMP_SRV_CONF|NGX_CONF_1MORE,
        ngx_rtmp_core_domains,
        NGX_RTMP_SRV_CONF_OFFSET,
        0,
    ...
    { ngx_string("load_conf_from"),
      ...
      NGX_RTMP_MAIN_CONF_OFFSET,
      offsetof(ngx_rtmp_core_main_conf_t, load_conf_from),
      &ngx_rtmp_core_load_conf_slots },
    ...
    { ngx_string("rtmp_status_code"),
        NGX_RTMP_MAIN_CONF|NGX_RTMP_SRV_CONF|NGX_CONF_TAKE1,
        ngx_conf_set_flag_slot,
        NGX_RTMP_SRV_CONF_OFFSET,
        offsetof(ngx_rtmp_core_srv_conf_t, rtmp_status_code),
        NULL },
    ...
    { ngx_string("publish_extime_fix"),
        NGX_RTMP_MAIN_CONF|NGX_RTMP_SRV_CONF|NGX_RTMP_APP_CONF|NGX_CONF_TAKE1,
        ngx_conf_set_flag_slot,
        NGX_RTMP_SRV_CONF_OFFSET,
        offsetof(ngx_rtmp_core_srv_conf_t, publish_extime_fix),
        NULL },
...
ngx_rtmp_core_create_srv_conf(ngx_conf_t *cf)
{
    ...
    if (ngx_array_init(&cscf->hls_play_domains, cf->temp_pool, 4,
    ...
    if (ngx_array_init(&cscf->rtmp_play_domains, cf->temp_pool, 4,
                sizeof(ngx_rtmp_server_name_t))
            != NGX_OK)
    {
        return NULL;
    ...
    if (ngx_array_init(&cscf->hdl_play_domains, cf->temp_pool, 4,
                sizeof(ngx_rtmp_server_name_t))
            != NGX_OK)
    {
        return NULL;
    ...
    if (ngx_array_init(&cscf->rtmp_publish_domains, cf->temp_pool, 4,
                sizeof(ngx_rtmp_server_name_t))
            != NGX_OK)
    {
        return NULL;
    ...
    cscf->publish_extime_fix = NGX_CONF_UNSET;
    cscf->buflen = NGX_CONF_UNSET_MSEC;
    cscf->busy = NGX_CONF_UNSET;
    cscf->rtmp_status_code = NGX_CONF_UNSET;
...
ngx_rtmp_core_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_value(conf->publish_extime_fix, prev->publish_extime_fix, 0);
...
static char *
ngx_rtmp_core_domains(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_rtmp_core_srv_conf_t *cscf = conf;
    u_char                    ch;
    ngx_str_t                *value;
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (value[0].data[0] == 'h') {
            if (value[0].data[1] == 'l') {
                sn = ngx_array_push(&cscf->hls_play_domains);
            } else {
                sn = ngx_array_push(&cscf->hdl_play_domains);
            }
        } else if (value[0].data[6] == 'l') {
            sn = ngx_array_push(&cscf->rtmp_play_domains);
        } else {
            sn = ngx_array_push(&cscf->rtmp_publish_domains);
        }
